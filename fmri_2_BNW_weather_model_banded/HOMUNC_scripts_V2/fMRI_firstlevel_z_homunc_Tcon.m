...
% clear R
load([dir_base '/fmrii_2_BNW_weather_model/HOMUNC_scripts_v2/beh2fmri_homunc_v8.mat']);

% No of movements and session covariates
covis = 8
nuisance   = 4;      % choice_lowP, choice_highP, outcome_lowP, outcome_highP
constant   = 1;
...
%     blockNo = size( runs{ n_use }, 2); % i.e. sessions
    ...
    %% set contrasts
    baseCols   = length(T.contrasts);
    totalCols = baseCols + nuisance + covis + constant;
    % Identity for the target-range regressors + zeros for the rest
    prepare_con = [ eye(baseCols), zeros(baseCols, totalCols - baseCols) ];
    u = size(prepare_con(1,:));   % keeps your downstream indexing intact
