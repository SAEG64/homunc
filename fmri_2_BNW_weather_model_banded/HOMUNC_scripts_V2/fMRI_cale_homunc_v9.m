...
beh_dat = 'DIRECTORY TO FMRI DATA: HOMUNC_parent/fmri_2_BNW_weather_model_banded/HOMUNC_scripts_V2/beh_v12_agg';
...
for i_sub = 1:length(sub_use)
    ...
    for i_ses = runs{ i_sub } % 1:size( R{ i_sub }.runs, 2)
        ...
        %% --- BAND-CODING for p(success) for banded GLM (no trial deletion) ---
        p_col       = 14;       % p(success) column
        weather_col = 7;        % weather (1 vs 2)

        p = Z.choice{i_ses}(:, p_col);
        w = Z.choice{i_ses}(:, weather_col);    % raw categorical weather (1/2)

        % Effect-code weather for GLM interpretation (keeps your factor structure)
        Z.weather_c{i_ses} = (w == 2) - 0.5;    % weather_c: W1 = -0.5, W2 = +0.5

        % Define band
        band_in  = (p >= 0.4) & (p <= 0.6);
        band_out = ~band_in;

        % Save band indicator (not required, but useful for QC and later HMM)
        Z.band{i_ses} = double(band_in);

        % --- p_in: only inside band (centered) ---
        p_in = zeros(size(p));
        if any(band_in)
            p_in(band_in) = p(band_in) - mean(p(band_in));
        end

        % --- p_out: only outside band, residualized relative to weather ---
        p_out = zeros(size(p));
        if any(band_out)
            X    = [ones(sum(band_out),1) w(band_out)];
            beta = X \ p(band_out);
            p_out(band_out) = p(band_out) - X*beta;
        end
        ...
        % (Optional but recommended) normalize only if non-zero variance
        if std(Z.p_in{i_ses}(band_in)) > 0
            Z.p_in{i_ses} = (Z.p_in{i_ses} - mean(Z.p_in{i_ses}(band_in))) ./ (std(Z.p_in{i_ses}(band_in)) + eps);
        end
        if std(Z.p_out{i_ses}(band_out)) > 0
            Z.p_out{i_ses} = (Z.p_out{i_ses} - mean(Z.p_out{i_ses}(band_out))) ./ (std(Z.p_out{i_ses}(band_out)) + eps);
        end
        ...
        del_opt_N_W1 = Z.choice_N{i_ses}(:,7) == 2
        del_opt_N_W2 = Z.choice_N{i_ses}(:,7) == 1
        del_opt_B_W1 = Z.choice_B{i_ses}(:,7) == 2
        del_opt_B_W2 = Z.choice_B{i_ses}(:,7) == 1
        del_opt_W_W1 = Z.choice_W{i_ses}(:,7) == 2
        del_opt_W_W2 = Z.choice_W{i_ses}(:,7) == 1
        
        % choices
        Z.choice_N_W1{i_ses} = Z.choice_N{i_ses};
        Z.choice_N_W2{i_ses} = Z.choice_N{i_ses};
        
        Z.choice_B_W1{i_ses} = Z.choice_B{i_ses};
        Z.choice_B_W2{i_ses} = Z.choice_B{i_ses};

        Z.choice_W_W1{i_ses} = Z.choice_W{i_ses};
        Z.choice_W_W2{i_ses} = Z.choice_W{i_ses};
        
        Z.choice_N_W1{i_ses}(del_opt_N_W1, :) = [];
        Z.choice_N_W2{i_ses}(del_opt_N_W2, :) = [];

        Z.choice_B_W1{i_ses}(del_opt_B_W1, :) = [];
        Z.choice_B_W2{i_ses}(del_opt_B_W2, :) = [];
        
        Z.choice_W_W1{i_ses}(del_opt_W_W1, :) = [];
        Z.choice_W_W2{i_ses}(del_opt_W_W2, :) = [];
        
        % outcomes
        Z.outcome_N_W1{i_ses} = Z.outcome_N{i_ses};
        Z.outcome_N_W2{i_ses} = Z.outcome_N{i_ses};
        
        Z.outcome_B_W1{i_ses} = Z.outcome_B{i_ses};        
        Z.outcome_B_W2{i_ses} = Z.outcome_B{i_ses};        

        Z.outcome_W_W1{i_ses} = Z.outcome_W{i_ses}; 
        Z.outcome_W_W2{i_ses} = Z.outcome_W{i_ses}; 
        
        Z.outcome_N_W1{i_ses}(del_opt_N_W1, :) = [];
        Z.outcome_N_W2{i_ses}(del_opt_N_W2, :) = [];

        Z.outcome_B_W1{i_ses}(del_opt_B_W1, :) = [];
        Z.outcome_B_W2{i_ses}(del_opt_B_W2, :) = [];
        
        Z.outcome_W_W1{i_ses}(del_opt_W_W1, :) = [];
        Z.outcome_W_W2{i_ses}(del_opt_W_W2, :) = [];
