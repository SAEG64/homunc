...
% No of movements and session covariates
covis = 8
...
%     blockNo = size( runs{ n_use }, 2); % i.e. sessions
    ...
    contrasts_FT = {'ons_overall','mod_overall','RT_overall','ons_main_weather','mod_main_weather','RT_main_weather','ons_main_ternary_1','ons_main_ternary_2','mod_main_ternary_1','mod_main_ternary_2','RT_main_ternary_1','RT_main_ternary_2','ons_interaction_1','ons_interaction_2','mod_interaction_1','mod_interaction_2','RT_interaction_1','RT_interaction_2'};
%     contrasts_triple = {'ons_N-WB','ons_W-NB','ons_B-NW','mod_N-WB','mod_W-NB','mod_B-NW','lRT_N-WB','lRT_W-NB','lRT_B-NW'};
%     F.contrasts = {'F_ons_NWB','F_mod_NWB','F_lRT_NWB'};
...
%     T.contrasts = [T.contrasts, contrasts_triple];
    T.contrastVectors = [ repmat(prepare_con, 1, blockNo) ];
%     F.contrastVectors = [ repmat(prep_F, 1, blockNo) ];
...
%     w_enc = 0;
%     for cont_no = 1:length(F.contrasts)
%         contrasttype = F.contrasts{cont_no};
%         contr_input_one     = F.contrastVectors(cont_no+w_enc,:);
%         contr_input_two     = F.contrastVectors(cont_no+w_enc+1,:);
% 
% %         jobs{0}.stats{1}.factorial_design.des.pt.pair.scans = cellstr(contr_input_one,contr_input_two);
%         jobs{1}.stats{1}.con.consess{k}.fcon = struct('name',contrasttype,'convec',[contr_input_one;contr_input_two],'sessrep','none')
%         w_enc = w_enc+1;
%         k = k+1
%     end
