...
for _, group in grouped:
    ...
    group['p_success'] = np.where(group['x7_weather_type_1'] == 1, 
                                  group['x60_weather_2_p_gain'],
                                  group['x59_weather_1_p_gain']).astype(np.float32)
    group['gain_magnitude'] = np.where(group['x7_weather_type_1'] == 1,
                                       group['x58_weather_2_gain_magnitude'],
                                       group['x57_weather_1_gain_magnitude']).astype(np.float32)
    ...
    while states.shape[0] < 5:
        ...
        # else:
        #     repeated_rows[:,-1] = 1
        repeated_rows[:, np.random.randint(0, 2, size=1)[0] + 3] = 1  # Random weather draws
        repeated_rows[:, 2] = np.where(repeated_rows[:, 4] == 1,
                     group.iloc[-1]['x60_weather_2_p_gain'].astype(np.float32),
                     group.iloc[-1]['x59_weather_1_p_gain'].astype(np.float32)).astype(np.float32)
    ...
    last_state = [
        group.iloc[-1]["x59_weather_1_p_gain"], group.iloc[-1]["x60_weather_2_p_gain"]] + [
            np.where(categ['x7_weather_type_1'] == 1,
                     group.iloc[-1]['x60_weather_2_p_gain'].astype(np.float32),
                     group.iloc[-1]['x59_weather_1_p_gain'].astype(np.float32)).astype(np.float32)] + [#list(
            np.where(categ['x7_weather_type_1'] == 1,
                     group.iloc[-1]['x58_weather_2_gain_magnitude'].astype(np.float32),
                     group.iloc[-1]['x57_weather_1_gain_magnitude'].astype(np.float32)).astype(np.float32)] + [#list(
        # list(categ.values())[1:]) + [
        0,
