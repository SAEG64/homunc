...
class ForestEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(
            low=0.0, high=5.0, shape=(11,), dtype=np.float32)
    ...
    def render(self, mode='human'):
        ...
        print(
            f"weather_bad: {obs[2]}, weather_good: {obs[3]}")
        print(
            f"ternary_0: {obs[4]}, ternary_0.5: {obs[5]}, ternary_1: {obs[6]}"),
        print(
            f"horizon: {obs[7]}, gain_weather_0: {obs[8]}, gain_weather_1: {obs[9]}")
        print(
            f"state: {obs[10]}")#, dead: {obs[11]}")
        print("======")
        print(
            f"done: {self.done}")
