...
class ForestEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.observation_space = spaces.Box(
            low=0.0, high=5.0, shape=(8,), dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        self.current_p = [self.weather_prob_0, self.weather_prob_1][int(self.current_weather)]
        self.food_gain = [self.energy_gain_0, self.energy_gain_1][int(self.current_weather)]
        self.ternary_state = 1  # Reset ternary state
        self.horizon = np.random.choice([3, 4, 5])       # Randomize forest duration
        self.horizon_adj = deepcopy(self.horizon)
        ...
        # self.dead = np.float32(0)         # Reset 'dead'
    ...
    def step(self, action):
        ...
        self.horizon_adj -= 1
        self.horizon_adj = np.clip(self.horizon_adj, 0, 5)
        self.current_day += 1
    ...
    def _get_obs(self):
        ...
        return np.array([
            ...
            self.energy_gain_0,
            self.energy_gain_1,
            self.current_energy], dtype=np.float32)#,
            # self.dead], dtype=np.float32)
