...
% v3: info on both weather types added 
clear
close all
...
H = load('HOMUNC_data_beh_B_fMRI_v3', 'header_columns');
...
L_load  = load([dir_base fs dir_scripts fs 'L_for_HOMUNC_list_v1.mat']);
L       = L_load.L_big;                                         % this is an "old" list going to be used below
L_REV   = load([dir_base fs dir_scripts fs 'L_REV_v2.mat']);    % this is a  "new" list going to be used below
...
for i_sub = 1:n_sub
    ...
    for i_line = 1:size( d ,1) % n of trials
        ...
        % v3: info on both weather types added
        d( i_line, 57:60) = L( old_scen, 5:8); % gains and probabilities for the two weather types
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Comment this to get full data sequence for HMM
    %d( d(:, 6) == 0, :)   = []; % if dead
...
% v3: info on both weather types added
header_columns{1,57} = '57_weather_1_gain_magnitude';
header_columns{1,58} = '58_weather_2_gain_magnitude';
header_columns{1,59} = '59_weather_1_p_gain';
header_columns{1,60} = '60_weather_2_p_gain';


%% B is the second step for the behavioral data: transformation
% here everything is saved
if which_sample == 1
    save('HOMUNC_data_beh_B_pilot_v3')
elseif which_sample == 2
    save('HOMUNC_data_beh_B_fMRI_v3')
end
